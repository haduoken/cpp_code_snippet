...
int main() {
  ...
  noiseModel::Diagonal::shared_ptr pricess = noiseModel::Diagonal::Sigmas(Vector3(1e-14, 1e-14, 1e-14));

  gra.push_back(BetweenFactor<Pose2>(1, 2, Pose2(2, 0, 0), model));
  gra.push_back(BetweenFactor<Pose2>(2, 3, Pose2(2, 0, M_PI_2 - 0.5), model));
  gra.push_back(BetweenFactor<Pose2>(3, 4, Pose2(2, 0, M_PI_2 - 0.5), model));
  gra.push_back(BetweenFactor<Pose2>(4, 5, Pose2(2, 0, M_PI_2 - 0.5), model));
  gra.push_back(BetweenFactor<Pose2>(5, 2, Pose2(2, 0, M_PI_2), model));
  gra.push_back(BetweenFactor<Pose2>(5, 2, Pose2(2, 0, M_PI_2), pricess));
  ...
  // 然后加入回环, 已经是有的位置了, 不需要添加initialEstimate
  graph.push_back(gra[5]);
  isam.update(graph);
  isam.update();
  Values currentEstimate = isam.calculateEstimate();
  cout << "****************************************************" << endl;
