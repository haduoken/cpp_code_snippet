...
int main() {
  ...
  // 添加odom, Noise 前三个是RPY(也是旋转轴的xyz), Noise的后三个是xyz
  //  gtsam::Vector Vector6(6);
  //  Vector6 << 1e-1, 1e-1, 1e-1, 1e-8, 1e-2, 1e-3;
  //  auto odom_noise = noiseModel::Diagonal::Variances(Vector6);
  //  graph.push_back(BetweenFactor<Pose3>(0, 1, GetPose3(1, 1, 1, 1, 0, 0), odom_noise));

  // 添加初始值
  initialEstimate.insert(0, GetPose3(0));
  //  initialEstimate.insert(1, GetPose3(2, 2, 2, 2));

  // 添加prior
  gtsam::Vector v_tmp_1(6), v_tmp_2(6);
  v_tmp_1 << 1e-1, 1e-1, 1e-1, 1e-1, 1e-1, 1e-1;
  v_tmp_2 << 1e-1, 1e-2, 1e-1, 1e-2, 1e-1, 1e-1;
  auto prior_1 = noiseModel::Diagonal::Variances(v_tmp_1);
  auto prior_2 = noiseModel::Diagonal::Variances(v_tmp_2);

  graph.push_back(PriorFactor<Pose3>(0, GetPose3(0, 0, 0, 0, 0, 0), prior_1));
  graph.push_back(PriorFactor<Pose3>(0, GetPose3(2, 2, 2, 2, 0, 0), prior_2));
  ...
  Pose3 pose0 = currentEstimate.at<Pose3>(0);

  auto roll = pose0.rotation().roll();
  auto pitch = pose0.rotation().pitch();
  auto yaw = pose0.rotation().yaw();
  cout << " final RPY is " << roll << " " << pitch << " " << yaw << endl;
