...
int main() {
  ...
  initialEstimate.insert(2, GetPose3(1.8));
  initialEstimate.insert(3, GetPose3(2.8));
  ...
  // odom 告诉机器人向x前进1m
  graph.push_back(BetweenFactor<Pose3>(0, 1, GetPose3(2), odom_noise));
  graph.push_back(BetweenFactor<Pose3>(1, 2, GetPose3(2), odom_noise));
  graph.push_back(BetweenFactor<Pose3>(2, 3, GetPose3(2), odom_noise));
  ...
  graph.push_back(GPSFactor(3, gtsam::Point3(0, 3, 0), gps_noise));
